...
class LSSToFoundryConverterV22:
    ...
    def _create_prototype_token(self, name, lss_character):
        # ═════════════════════════════════════════════════════════════════════════════════
        # ПАРАМЕТРЫ ТОКЕНА ДЛЯ FOUNDRY VTT (D&D 5e v4.0+)
        # 
        # Этот метод создаёт стандартный прототип токена для персонажа. Видение
        # корректируется в зависимости от настроек пользователя (раса, чекбоксы).
        # Остальные параметры используют оптимальные значения для игровых персонажей.
        # ═════════════════════════════════════════════════════════════════════════════════

        return {
            # ОСНОВНОЕ ИМУЩЕСТВО
            # Имя токена (отображается над фишкой на сцене)
            "name": name,

            # ОТОБРАЖЕНИЕ ИМЕНИ (displayName):
            # 0  = НИКОГДА не показывать имя
            # 10 = ВЛАДЕЛЬЦАМ персонажа (игрокам)
            # 20 = ВСЕМ на сцене (GM + игроки) ← ИСПОЛЬЗУЕТСЯ
            # 30 = ТОЛЬКО GM
            # ВЫБРАНО: 20 - видно всем (стандарт для PC-персонажей)
            "displayName": 20,

            # СВЯЗЬ ТОКЕНА С АКТЁРОМ (actorLink):
            # False = токен НЕЗАВИСИМ (это копия данных персонажа)
            #         изменения на токене не влияют на актёра в Directory
            # True  = токен ЛИНКОВАН с актёром (связан, как ссылка) ← ИСПОЛЬЗУЕТСЯ
            #         изменения HP, статуса видны сразу на актёре
            # ВЫБРАНО: True - удобнее для PC-персонажей (всё синхронизируется)
            "actorLink": True,

            # РАЗМЕР ТОКЕНА
            # width/height = количество клеток по ширине и высоте
            # 1 = Medium (стандартный размер)
            # 2 = Large, 3 = Huge и т.п.
            "width": 1,
            "height": 1,

            # ИЗОБРАЖЕНИЕ ТОКЕНА
            "texture": {
                # Путь к картинке токена (по умолчанию стандартный аватар)
                "src": "icons/svg/mystery-man.svg",

                # ТОЧКА ПРИВЯЗКИ (anchorX, anchorY):
                # Диапазон: 0.0 - 1.0 (где 0.5 = центр)
                # 0.5/0.5 = центр картинки совпадает с центром клетки
                "anchorX": 0.5,
                "anchorY": 0.5,

                # СМЕЩЕНИЕ КАРТИНКИ (offsetX, offsetY):
                # В пикселях. Смещает картинку внутри клетки
                "offsetX": 0,
                "offsetY": 0,

                # РЕЖИМ МАСШТАБИРОВАНИЯ (fit):
                # "contain" = картинка целиком влезает в клетку (без обрезки)
                # "cover"   = картинка заполняет клетку, может обрезаться
                "fit": "contain",

                # МАСШТАБ (scaleX, scaleY):
                # 1.0 = 100% оригинального размера
                # 0.5 = 50%, 2.0 = 200% и т.п.
                "scaleX": 1,
                "scaleY": 1,

                # ПОВОРОТ ИЗОБРАЖЕНИЯ (rotation):
                # В градусах (0-360). Поворачивает саму картинку в клетке
                "rotation": 0,

                # ЦВЕТОВОЙ ТИНТ (tint):
                # HEX код. Накладывается поверх картинки
                # "#ffffff" = белый (нейтральный, без изменений)
                "tint": "#ffffff",

                # ПОРОГ АЛЬФЫ (alphaThreshold):
                # 0.0-1.0. Пиксели с прозрачностью ниже этого значения
                # считаются полностью прозрачными (не влияют на границы)
                "alphaThreshold": 0.75
            },

            # БЛОКИРОВКА ВРАЩЕНИЯ (lockRotation):
            # False = токен можно поворачивать мышью / анимациями
            # True  = токен ЗАБЛОКИРОВАН и не поворачивается ← ИСПОЛЬЗУЕТСЯ
            # ВЫБРАНО: True - для гуманоидов обычно не нужен поворот
            "lockRotation": True,

            # НАЧАЛЬНЫЙ ПОВОРОТ ТОКЕНА (rotation):
            # В градусах. Начальная ориентация при создании
            "rotation": 0,

            # ОБЩАЯ ПРОЗРАЧНОСТЬ (alpha):
            # 0.0 = полностью прозрачный
            # 1.0 = полностью непрозрачный
            "alpha": 1,

            # ОТНОШЕНИЕ К ИГРОКАМ (disposition):
            # -1 = ВРАЖДЕБНЫЙ (красный) - враги, монстры
            #  0 = НЕЙТРАЛЬНЫЙ (жёлтый) - NPC, животные
            #  1 = ДРУЖЕСТВЕННЫЙ (голубой) - союзники, PC ← ИСПОЛЬЗУЕТСЯ
            # ВЫБРАНО: 1 - персонажи игроков (дружественные)
            "disposition": 1,

            # ОТОБРАЖЕНИЕ ПОЛОСОК (displayBars):
            # Показывает ли полоски HP/ресурсов над токеном
            # 0  = НИКОГДА не показывать
            # 10 = ВЛАДЕЛЬЦАМ (только игроку)
            # 20 = ВСЕМ на сцене ← ИСПОЛЬЗУЕТСЯ
            # 30 = ТОЛЬКО GM
            # ВЫБРАНО: 20 - видно всем (удобно видеть HP врагов и союзников)
            "displayBars": 20,

            # ПЕРВАЯ ПОЛОСКА (bar1):
            # К какому атрибуту привязана
            # "attributes.hp" = полоска HP (стандарт для персонажей)
            "bar1": {"attribute": "attributes.hp"},

            # ВТОРАЯ ПОЛОСКА (bar2):
            # None = не привязана ни к чему
            # Можно привязать к: "resources.primary", "attributes.ac" и т.п.
            "bar2": {"attribute": None},

            # ВСТРОЕННЫЙ СВЕТ ТОКЕНА (light)
            # Параметры источника света, излучаемого токеном
            "light": {
                # НЕГАТИВНЫЙ СВЕТ (negative):
                # False = обычный свет (добавляет яркость)
                # True  = темный свет (вычитает, как аура тьмы)
                "negative": False,

                # ПРИОРИТЕТ РЕНДЕРА (priority):
                # 0 = обычный приоритет
                # > 0 = рисуется поверх других источников света
                "priority": 0,

                # ПРОЗРАЧНОСТЬ СВЕТА (alpha):
                # 0.0-1.0. Интенсивность самого источника
                "alpha": 0.5,

                # УГОЛ ОСВЕЩЕНИЯ (angle):
                # В градусах. 360 = круговой свет
                # 90 = конус 90°, 180 = полукруг и т.п.
                "angle": 360,

                # РАДИУС ЯРКОГО СВЕТА (bright):
                # В клетках сцены (Foundry v11+)
                # 0 = нет яркого света, только тусклый
                "bright": 0,

                # ЦВЕТ СВЕТА (color):
                # HEX код или None (белый по умолчанию)
                "color": None,

                # СТИЛЬ ОКРАСКИ (coloration):
                # Числовое значение, зависит от версии Foundry
                # Обычно 1 = стандартная цветокоррекция
                "coloration": 1,

                # РАДИУС ТУСКЛОГО СВЕТА (dim):
                # В клетках сцены. Дополнение к bright radius
                "dim": 0,

                # ЗАТУХАНИЕ (attenuation):
                # 0.0-1.0. Насколько быстро свет гаснет к краям
                # 0.5 = линейное затухание
                "attenuation": 0.5,

                # ЯРКОСТЬ (luminosity):
                # -1.0 до 1.0
                # 0.5 = обычный свет
                # < 0 = темный свет (вычитает освещение)
                "luminosity": 0.5,

                # НАСЫЩЕННОСТЬ (saturation):
                # -1.0 до 1.0. Интенсивность цвета
                # 0 = обычная, > 0 = более насыщенный
                "saturation": 0,

                # КОНТРАСТ (contrast):
                # -1.0 до 1.0. Контраст света
                "contrast": 0,

                # ТЕНИ (shadows):
                # -1.0 до 1.0. Интенсивность теней в свете
                "shadows": 0,

                # АНИМАЦИЯ СВЕТА (animation)
                # Эффекты мерцания, пламени, волн и т.п.
                "animation": {
                    # Тип анимации:
                    # None = без анимации
                    # "torch", "pulse", "chroma", "wave", "fog", "sunburst" и др.
                    "type": None,

                    # СКОРОСТЬ анимации (0-10)
                    # Выше число = быстрее
                    "speed": 5,

                    # ИНТЕНСИВНОСТЬ эффекта (0-10)
                    # Амплитуда колебаний/мерцания
                    "intensity": 5,

                    # ОБРАТНОЕ ВОСПРОИЗВЕДЕНИЕ (reverse):
                    # True = проигрывать анимацию в обратную сторону
                    "reverse": False
                },

                # ТЬМА (darkness):
                # Параметры видимости в темноте
                "darkness": {"min": 0, "max": 1}
            },

            # ═════════════════════════════════════════════════════════════════════════════════
            # ВИДЕНИЕ ТОКЕНА (sight)
            # 
            # Это ДИНАМИЧЕСКИЙ параметр, зависит от выбора пользователя:
            # - Раса (стандартное видение)
            # - Чекбокс "Взор дьявола" (добавляет darkvision 120 ft)
            # - Чекбокс "Боевой стиль Слепой бой" (blindsight 10 ft)
            # - Ручное переопределение (если пользователь выбрал в expander)
            # 
            # Метод _create_sight_config() возвращает правильные значения
            # на основе self.vision_config, установленного перед вызовом.
            # ═════════════════════════════════════════════════════════════════════════════════
            "sight": self._create_sight_config(),

            # РЕЖИМЫ ОБНАРУЖЕНИЯ (detectionModes):
            # Пусто по умолчанию (используются режимы видения из sight)
            "detectionModes": [],

            # ОККЛЮЗИЯ (occludable):
            # Может ли свет блокировать стены
            "occludable": {"radius": 0},

            # КОЛЬЦО вокруг токена (ring):
            # Выделение токена на сцене
            "ring": {
                "enabled": False,
            ...
            # МАРКЕР ХОДА (turnMarker):
            # Маркер текущего хода в боевом раунде
            "turnMarker": {
                "mode": 1,
                "animation": None,
                "src": None,
